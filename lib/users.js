...
exports.userExists = function(username, createIfNotExists, callback) {
    database.getDatabaseConnection(function (mysql_client) {
        mysql_client.query("SELECT * FROM users WHERE username='" + escape(username) + "';", function (error, results, fields) {
            if(error) {
                throw "Error verifying is user exists: " + error;
            }
            if(results.length > 0) {
                callback(true, results[0].id);
            } else {
                if(createIfNotExists) {
                    exports.createUser(1, username, "Imported from WordPress blog", username, username, username, function(userId){
